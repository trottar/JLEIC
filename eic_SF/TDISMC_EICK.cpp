...
Double_t GetF2kForCut(Int_t NEvts, Int_t IndexData[], Double_t xBjData[],
...
int mainxk(double xMin,double xMax, double Q2Min,double Q2Max, double rnum) {
  ...
  //  Int_t rnumber = gRandom->Uniform(10000); // big number in (<10000000000)
	...
	sprintf(tTitle,"p(e,e'Lambda)X Event Generation %3.0f GeV/c x%4.0f GeV/c",
			kBeam, PBeam);
	//	sprintf(rName,"TDIS-MC%02.0fx%03.0f.root", kBeam, PBeam);
	sprintf(rName,"TDISkaon.root");
	//	TFile *fRoot = TFile::Open(rName,"Recreate");
	TFile fRoot(rName,"Recreate", tTitle);
	//sprintf(tName,"TDISkaon_MC%02.0fx%03.0f", kBeam, PBeam);
	...
	tree->Branch("invts",&invts,		     "s_e/D:s_q/D:Q2/D:xBj/D:nu/D:W/D:x_D/D:y_D/D:tSpectator/D:tPrime/D:TwoPdotk/D:TwoPdotq/D:p_RT/D:pDrest/D:tempVar/D:MX2/D:alphaS/D:pPerpS/D:pPerpZ/D");
	...
	//GEMC, commented out below
	
	//name of output file : = "TDIS_lund.txt";
	ofstream OUT ("TDIS_lund_ek.txt", ios::app);
	...
	for (int iEvt=0; iEvt<NEvts; iEvt++) {
	  ...
	  IndexData[iEvt] = iEvt;
		...
		*/
		...
		//cout << "debugging: " << "p2_pt(GeV/c) = " << p2_pt << ", p2_z(GeV/c) " << p2_z << endl;
		...
		*/
		...
		/*
		if((TDIS_xbj >0.18&& TDIS_xbj <0.19)){
		  cout << " what is random number (uu)= " <<  invts.Q2  << endl;
		}
		*/
		...
		if (TDIS_xbj > 0.0 && TDIS_xbj < 1.0){
		  ...
		  // for Kaon SF
		  //  typ = 2 ! s-dependent expon FORM FACTOR (L=1.293,dis=0,FLAG=0)
		    // using same GRV(pion PDFs), piNN coupling constant,

		  // Debug
		  //cout << "Debug: " << TDIS_xbj << ", Q2 ="<< invts.Q2 << endl;
		  //
		  
		  fpi = fpifac*f2kp(P_pi, TDIS_xbj, theta_p2/D2R);
		  // typ=5  ! t-dependent monopole Form Factor (regularization form) (L=0.928,FLAG=0)
                  //fpi = fpifac*f2kptmono(P_pi, TDIS_xbj, theta_p2/D2R);
		  ...
		  */
		  ...
		  //cout << "test outout GRV xVpi = " << TDIS_xbj <<", Q2= " << invts.Q2 << ", fpi " << fpi  <<endl;
		...
		*/
		...
		if (TDIS_xbj > 0. && TDIS_xbj < 1.0) {
		  ...
		  // #### important !!!!
		  // cross-section unit "nanobarn" will be applied later... 10^-33 cm2
		...
		*/
		...
		GQ2->SetPoint(iEvt,invts.xBj,invts.Q2);
		//Gf2N->SetPoint(iEvt,invts.Q2,f2N);
		//cout << Form("%d\t%4.2f\t%4.2f\t%4.2f",IndexData[iEvt],xBjData[iEvt],Q2Data[iEvt],F2NData[iEvt]) << endl;
		
		
		hnu->Fill(qVirtual_Rest.E());


		//TGraph *gq2x = New TGraph(3500,invts.xBj,invts.Q2);
       ...
       OUT << setiosflags(ios::left) << setiosflags(ios::fixed) <<  "\t" << "2" << " \t " << pr_particle_charge << " \t " << " 1 "<< " \t " << pr_particle_id << " \t " << "0" <<  " \t "<< "1" << " \t "<< scientific << PBeamMCx+PBeamMC*sin(CrossingTheta)*cos(CrossingPhi) << " \t " << PBeamMCy+PBeamMC*sin(CrossingTheta)*sin(CrossingPhi) << " \t " << PBeamMC*cos(CrossingTheta) << " \t " << PBeamMC << " \t " << MProton << " \t " << vd0X_Lab  << " \t " << vd0Y_Lab << " \t " << vd0Z_Lab << endl; 
       ...
       //       OUT << setiosflags(ios::left) << setiosflags(ios::fixed) <<  "\t" << "5" << " \t " << pi_particle_charge << " \t " << " 1 "<< " \t " << pi_particle_id << " \t " << "0" <<  " \t "<< "1" <<  " \t "<< scientific<< ppix_Lab << " \t " << ppiy_Lab << " \t " << ppiz_Lab << " \t " << EpiE_Lab << " \t " << mPion << " \t " << vpix_Lab  << " \t " << vpiy_Lab << " \t " << vpiz_Lab << endl; 
       // the first spectator protn (TDIS)
       OUT << setiosflags(ios::left) << setiosflags(ios::fixed) <<  "\t" << "4" << " \t " << sp_particle_charge << " \t " << " 1 "<< " \t " << sp_particle_id << " \t " << "0" <<  " \t "<< "1" <<  " \t "<< scientific<< pprx_Lab << " \t " << ppry_Lab << " \t " << pprz_Lab << " \t " << EprE_Lab << " \t " << spmass << " \t " << vprx_Lab  << " \t " << vpry_Lab << " \t " << vprz_Lab << endl; 
	...
	double xminbin[20]={0.01,0.0125,0.0158,0.0199,0.0251,0.0316,0.0398,0.0501,0.0631,0.0794,0.1,0.125,0.158,0.199,0.251,0.316,0.398,0.501,0.631,0.794};
	double xmaxbin[20]={0.0125,0.0158,0.0199,0.0251,0.0316,0.0398,0.0501,0.0631,0.0794,0.1,0.125,0.158,0.199,0.251,0.316,0.398,0.501,0.631,0.794,1.00};
	
	double q2minbin[10]={1.0,1.58,2.51,3.98,6.31,10.0,15.8,25.1,39.8,63.1};
	double q2maxbin[10]={1.58,2.51,3.98,6.31,10.0,15.8,25.1,39.8,63.1,100.};

	Double_t F2kMean;
	Double_t q2BinMean;
	Double_t F2kMeanError;
	...
	for (int i = 0; i < 20; i++) {
	  GCf2N->SetTitle(Form("F2k vs Q^{2} xBj[%4.4f:%4.4f]",xminbin[i],xmaxbin[i]));
	  for (int j = 0; j < 10; j++) {
	    ...
	    F2kMeanError = uncerW(F2kMean,i,uncerVal);
	    F2kMean = GetF2kForCut(NEvts,IndexData,xBjData,Q2Data,F2NData,q2minbin[j],q2maxbin[j],xminbin[i],xmaxbin[i],F2kMeanError);	    
	    GCf2N->SetPoint(j, (q2minbin[j] + q2maxbin[j])/2, F2kMean);
	    //GCf2N->SetPointError(j,0.,F2kMeanError); //FIXME
	  }
	  GCf2N->GetXaxis()->SetTitle("Q^{2}");
	  GCf2N->GetYaxis()->SetTitle("f2KMean");
	  GCf2N->Draw("AP");
	  CG->Print(Form("F2kvsQ2__XFrom%4.3f_TO%4.3f.png",xminbin[i],xmaxbin[i]));
	}
	...
	TMath::Sort(NEvts,XDataQ2,IndOrderQ2,false);
	TMath::Sort(NEvts,XDataf2N,IndOrderf2N,false);
	
	Double_t XCurveQ2[MEvts];
	Double_t YCurveQ2[MEvts];
	Double_t XCurvef2N[MEvts];
	Double_t YCurvef2N[MEvts];

	Int_t Aux1 = 0;
	Int_t Aux2 = 0;
	for (int iEvt=0; iEvt<NEvts; iEvt++){
	  if( XDataQ2[IndOrderQ2[iEvt]] != 0 && YDataQ2[IndOrderQ2[iEvt]] != 0 ){
	    XCurveQ2[Aux1] = XDataQ2[IndOrderQ2[iEvt]];
	    YCurveQ2[Aux1] = YDataQ2[IndOrderQ2[iEvt]];
	    //cout << XCurveQ2[Aux] << "\t" << YCurveQ2[Aux] << endl;
	    Aux1++;
	  }

	  if( XDataf2N[IndOrderf2N[iEvt]] != 0 && YDataf2N[IndOrderf2N[iEvt]] != 0 ){
	    XCurvef2N[Aux2] = XDataf2N[IndOrderf2N[iEvt]];
	    YCurvef2N[Aux2] = YDataf2N[IndOrderf2N[iEvt]];
	    //cout << XCurvef2N[Aux2] << "\t" << YCurvef2N[Aux2] << endl;
	    Aux2++;
	  }
	...
	CGA->Print(Form("Q2vsxBj2DHisto_PBeam%4.2f_kBeam%4.2f_Q2Max%4.2f.png",PBeam,kBeam,Q2Max));
...
TH1F *HF2kCut = new TH1F("HF2kCut", "F2K1D_Histo", 100, 0.0, 1.0);

Double_t GetF2kForCut(int NEvts, Int_t IndexData[], Double_t xBjData[],
		      Double_t Q2Data[], Double_t F2NData[],
		      Double_t Q2CutMin, Double_t Q2CutMax,
		      Double_t XCutMin, Double_t XCutMax, Double_t& MeanError){
  ...
  CHf2->cd();
  HF2kCut->Draw();
  CHf2->Print(Form("f2k1DHisto_Q2From%4.3f_To%4.3f__XFrom%4.3f_To%4.3f.png",Q2CutMin,Q2CutMax,XCutMin,XCutMax));

  MeanError = HF2kCut->GetMeanError();
  return HF2kCut->GetMean() ;
