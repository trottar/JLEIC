...
int mainx(double xMin,double xMax, double Q2Min,double Q2Max, double rnum){
  ...
  //ran2.SetSeed(rnumber);
	...
	sprintf(tTitle,"p(e,e'np)X Event Generation %3.0f GeV/c x%4.0f GeV/c",
			kBeam, PBeam);
	//sprintf(rName,"TDIS-MC%02.0fx%03.0f.root", kBeam, PBeam);
	sprintf(rName,"TDISpion.root");
	//	TFile *fRoot = TFile::Open(rName,"Recreate");
	TFile fRoot(rName,"Recreate", tTitle);
	//sprintf(tName,"TDISpion", kBeam, PBeam);
	...
	// double xminbin[20]
	//   ={0.01,0.0125,0.0158,0.0199,0.0251,0.0316,0.0398,0.0501,0.0631,0.0794,0.1,0.125,0.158,0.199,0.251,0.316,0.398,0.501,0.631,0.794};
	// double xmaxbin[20]
	//   ={0.0125,0.0158,0.0199,0.0251,0.0316,0.0398,0.0501,0.0631,0.0794,0.1,0.125,0.158,0.199,0.251,0.316,0.398,0.501,0.631,0.794,1.00};
