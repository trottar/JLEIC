...
C  PION AND RHO
C
C  MAINLY, THESE ARE FOR THE REACTION e + n --> Lambda + e' + X, AS
...
C  MODIFIED: Nov. 04, 2016 K. Park for Kaon SF.
c all pion related varables are replaced with kaon (no change variable name but values !!)
C **********************************************************************
C VARIABLE DECLARATIONS
	IMPLICIT NONE
	INTEGER ix,nx,ith,ik,ikTx,FLAG,typ,dis
	PARAMETER (nx=100)
	EXTERNAL fyKY,fypiN, f_rhoN, f_RhoDel, fypiD, CTQ6PDF
	REAL*8  fyKY,fypiN, f_rhoN, f_RhoDel, fypiD
	...
	REAL*8  minQ2,maxQ2,ntQ2
	INTEGER nq2,iqq
	PARAMETER (nq2=10)
C***********************************************************************
!---------------------------------------------------------------------
!WE SET THE RENORMALIZATION CUT-OFF PARAMETER LAMBDA "L" = ... IN UNITS OF GeV
!	L = 1.293  ! this is for choice of s-dependent exp  arXiv:1610.03333v1
	L = 0.928  ! this is for choice of t-dependent monopole.
! 	L = 0.545  ! this is for choice of Pauli-Villar. - Use this for Kaon as well.
...
!        typ = 2 !s-dependent EXPONENTIAL regularization form
!       typ = 3 !COV. DIPOLE FORM FACTOR
	typ = 5			!t-dependent monopole regularization FACTOR
!	typ = 6			!Pauli-Villar regularization FACTOR
C***********************************************************************
...
!THIS CHOOSES AMONG THE VARIOUS POSSIBLE COMBINATIONS OF SPLITTING FUNCTION/PDF
      FLAG = 0
!THE FLAGS TOGGLE AMONG DISSOCIATION MODES AS: single mode
...
! Configuration of EIC: given by xBj and Q2 directly
...
!  CALCULATE USEFUL KINEMATICAL VARIABLES:
       mN = 0.93891897D0          !TARGET Nucleon MASS, GeV
!!	     Q2 = 2.D0*mN*x*E	![GeV]**2
!!     & * (1.D0 - 1.D0/( (2.D0*E/(x*mN)) 
!!     & * DSIN((theta_e/180.D0)*pi)**2.D0 + 1.D0 ) )
	
!	ntQ2 = (maxQ2-minQ2)/nQ2
!	DO iqq = minQ2,maxQ2+ntQ2/10,ntQ2
!
	iqq = 10.  ! Q2 = 10GeV2
!      THERE IS NO Q2 DEPENDENCE SO, NO NEED THIS LOOP....
!
...
!	  kmax = 0.50D0		!SHARP k BOUNDS (FROM THIA!) [GeV]
	  kmin = 5.D0
	  kmax = 10.0D0		!SHARP k BOUNDS (FROM THIA!) [GeV]
	  kint = (kmax-kmin)/100.D0
!

!        !!!! momentim integration 
!
	  DO k=kmin,kmax,kint   
!
!	     
...
!            pi_int = rho_PS*fypiN(y,kTk,L,typ,dis) * F2pi_GRVT
            pi_int = rho_PS*fyKY(y,kTk,L,typ,dis) * F2pi_GRVT
                                                !PION CONTRIBUTION
          ...
          ENDIF
c	  if(pi_int.gt.0..AND.y.gt.0.06.AND.y.lt.0.07)then
c	     print *,'y= ',y,' pi_int = ',pi_int
c	  endif
...
!----- FOR THE |k| INTEGRAL
	  ...
	  ENDDO  !!!! momentum integration loop


	    F2pi_k = (kint/3.D0) * F2pi_k

!	    print *,'xBj= ',x,', Q2=',iQQ,', ',
!     & 'p_k= ',k,' GeV/c',', thetaK=',the_h*180/3.14,', F2piK= ',F2pi_k
...
!----- AND AGAIN FOR THE theta_h INTEGRAL
	  ...
	  ENDDO    !!!! theta angle integration loop


	    F2piK(ix) = (thint/3.D0) * F2piK(ix)

!        print*, 'x= ',x, '   F2piK =',F2piK(ix)
        print*,x,' ',F2piK(ix)
!!!! This output should be used for parametrization (x, k, theta)
!        print*, x,', ',iQQ,',  ',F2piK(ix)
...
c	mN  = 1.1156D0 ! Lambda masses in GeV!!
          IF (dis.EQ.0) THEN
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!!**** THE DISSOCIATION P --> L + K^+, 
	mpi = 0.13957018D0    !THE MASS OF THE K^+
c	mpi = 0.493667D0    !THE MASS OF THE K^+
	mP = mN               !THE MASS OF THE NUCLEON
        g_piNN = DSQRT (14.40D0 * 4*pi)    ! g_{piNN} 
	gg = 2.D0 * g_piNN**2 / (16.D0 * pi**2)  ! 1 ISOSPIN FACTOR
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
          ELSE IF (dis.EQ.1) THEN
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!!**** THE DISSOCIATION P --> P + pi^0  this is not relavant for K\Lambda
...
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	ENDIF
          ...
          ELSE IF (typ.EQ.2) THEN
            FF = DEXP( (mN**2 - SpiN)/(L**2) )          ! s-dependent expon testing 11/08/2016
     ...
     &                         + y*mP**2.D0)/(1.D0-y) + mN**2.D0
          ...
          ELSE IF (typ.EQ.5) THEN
            t = (- kT2 - mN**2*y**2) /(1.D0-y)
            FF =  DEXP( ( t- mpi**2)/(L**2) )      ! t-dependent exp
          ELSE IF (typ.EQ.6) THEN
            t = (- kT2 - mN**2*y**2) /(1.D0-y)
            FF =  DSQRT(1- ( t- mpi**2)**2/(t-L**2)**2)      ! Pauli-Villars
          ENDIF
...
!!**** THE DISSOCIATION N ---> (pi^0 + DELTA^0) + (pi^- + DELTA^+)
	...
	mpi =  13957018D0 !Mass OF THE pion
...
!	gg =  (2.D0/3.D0) * g_DLcN**2 / (16.D0 * pi**2)   !2/3 ISOSPIN FACTOR
	g_DLcN = DSQRT (0.22370D0 * 4*pi)    ! same as piNN (just Don't know)
	gg =  (2.D0/3.D0)* g_DLcN**2 / (16.D0 * pi**2)   !2/3 ISOSPIN FACTOR
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
          ELSE IF (dis.EQ.1) THEN
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
C ***************************************************************************
C ***********************************************************************
	FUNCTION fyKY (y,kT,L,typ,dis)
C
C  Function giving numerical value of f(y) for the SU(4) analogue of the KY
C  interaction, as usual y is IMF momentum fraction of the charmed BARYON.
C ***********************************************************************
	IMPLICIT NONE
	INTEGER typ,dis
	REAL*8  ss,kT,kT2,SpiD
	REAL*8  y,yR,fyKY,t
	REAL*8  pi,mN,mY,mK,g_DLcN,gg,FF,L,sM
...
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!!**** THE DISSOCIATION N ---> (pi^0 + DELTA^0) + (pi^- + DELTA^+)
	mY = 1.11562D0  !THE MASS OF THE Lambda Hyperon
	mK =  0.493667D0  !Mass OF THE KAON 
!!*** WE USE THE COUPLINGS INFERRED FROM HAIDENBAUER ET AL. ***
!	g_DLcN = DSQRT (0.2237D0 * 4*pi)    ! as N-pi-Del from Holzenkamp et al.
!	gg =  (2.D0/3.D0) * g_DLcN**2 / (16.D0 * pi**2)   !2/3 ISOSPIN FACTOR

	g_DLcN = -13.3 ! coupling constant (Kp\Lambda) model-dependent (range: piNN=13.1-13.5)
!	g_DLcN = -3.5 ! coupling constant (Kp\Sigma0)
!                                       ! S. Goloskokovand P. Kroll, Eur.Phys.J. A47(2011) 112
! Therefore even pion+NN coupling constant is still valid for K\Lambda
!	g_DLcN = DSQRT (14.40D0 * 4*pi)    ! coupling constant (piNN)
!              = 13.4486 (sign is different but eventually square term will be used !!)
	gg =   g_DLcN**2 / (16.D0 * pi**2)   !2/3 ISOSPIN FACTOR
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
          ELSE IF (dis.EQ.1) THEN
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	ENDIF
	  ...
	  kT2 = kT**2
	  SpiD = (kT2 + mK**2)/y + (kT2 + mY**2)/(1.D0-y)
!********************************************************************************
          IF (typ.EQ.0) THEN
            FF = ((L**2 + mN**2) / (L**2 + SpiD))       ! monopole
          ELSE IF (typ.EQ.1) THEN
            FF = ((L**2 + mN**2) / (L**2 + SpiD))**2    ! dipole
          ELSE IF (typ.EQ.2) THEN
            FF = DEXP( (mN**2 - SpiD)/(L**2) )          ! s-channel expon
          ELSE IF (typ.EQ.3) THEN
            t = (- kT2 - mN**2*y**2) /(1.D0-y)
            FF = ((L**2 - mK**2) / (L**2 - t))**2      ! cov dip
          ELSE IF (typ.EQ.4) THEN                       ! DIPOLE -- s-channel Lambda exchange
            sM = (kT2 + (1.D0+y)*mK**2.D0)/y + (kT2 
     &                         + y*mY**2.D0)/(1.D0-y) + mN**2.D0
            FF = (L**4 + mY**4)/(L**4 + sM**2) 
          ELSE IF (typ.EQ.5) THEN
            t = (- kT2 - mN**2*y**2) /(1.D0-y)
            FF = ((L**2 - mK**2) / (L**2 - t))**2       !  t-dependent monopole
          ENDIF

         yR = 1.D0 - y
      ss = ( kT2 + (mY-yR*mN)**2) * ( kT2 + (mY+yR*mN)**2 )**2
     &       / ( 6.D0*mY**2*yR**3) / ( (1.D0-yR)*(SpiD - mN**2) )**2 
     &       * FF**2

	fyKY =  gg/(mK**2) * (1.D0-yR) / yR * ss
