 ...
 *  TDISMC_EICn
 * -- This is for neutron tagging on either TDIS spectator or STEG spectator
 * ep-> e(n)\pi+ or eD->e(p)\pi0+(n) :  ( ) tagging particle(s)
 */
#include <stdio.h>
#include <TH1.h>
#include <TH1D.h>
...
/*
 In the code, there is a function (defined at Eq.(3) in your article, arXive:1411.2216v2) 
which describes the light-cone momentum distribution of pion in the nucleon. 
This function actually goes into the convolution function for the contribution to 
inclusive F2 from the scattering off of the virtual pion.
 At this point, (Please, correct me if I am wrong...or mis-understanding something...) 
If I take look into Eq.(3), there is a "G_{\piNN}" that is the parameterization of the 
momentum dependent \piNN vertex function. This is related to the "regularization prescription"
 in the pion exchange model (J.Mckenney, PRD 93, 054011(2016)). 
I believe this parametrization of f2pi() is done by "Timothy John Hobbs"

 An assumption of the s-dependent form-factor ( F=exp[(M**2-s)/Lambda**2]). 
Since this form-factor is based the model dependent ansatz, it is interesting to see any difference
 in the simulation by change from "s-dependence" to "t-dependence" (for example, F=exp[(t-mpi**2)/Lambda**2]) 
 OR by change to other form-factor (t-dependent monopole or Regge exp., ...).
*/
 ...
 *  Modification to F2ALLM to also output deuterium using a Q^2 dependent correction for n/p
 */

double f2allm_f2d_q2dep(double xin, double q2in)
{
  double M02,M12,LAM2,M22;
  double ALLM,SP,AP,BP,SR,AR,BR,S,XP,XR,F2P,F2R;
  double xx;
  double s11,s12,s13,s21,s22,s23,a11,a12,a13;
  double b11,b12,b13,a21,a22,a23,b21,b22,b23;
  double f2temp,f2allm,xb,q2,q02,alfa,xmp2,w2,w;
  double z,mp,ppp,alpha,x,f2allm_f2h,no,f2allm_f2d_q2dep;
  double f2nf2p;

  // redefine the input variable
  xb = xin;
  x = q2in;
  no = 1.0; 
 ...
 const double B11=0.60243*0.60243;
 const double B12=1.3754*1.3754;
 ...
 const double B21   =   0.10711*0.10711;
 const double B22   =   1.9386*1.9386;
 const double B23   =   0.49338;
 M22   =   0.15052 ;
	 //
 M02=0.31985;
 LAM2=0.065270;
 double Q02=0.46017+LAM2;        
 const double ALFA=112.2;
 const double XMP2=0.8802;
 double W2,Q2,W,Z;
  ...
  if(Q2==0.){
    ...
    f2allm_f2h=no*q2/(q2+M02)*(F2P+F2R);
      
    f2nf2p=(0.979+xb*(-0.286+xb*(-1.554+xb*(3.704+xb*(-2.443))))+(0.00297-0.0626*xb+0.0968*pow(xb,2))*log(q2))*2-1;

    // Modification to F2ALLM to also output *DEUTERIUM* using a Q^2 dependent correction for n/p
    // F2 deterium with Q2 evolution correction for n/p
    f2allm_f2d_q2dep=f2allm_f2h*0.5*(f2nf2p+1);
    
    return f2allm_f2d_q2dep/F2P;
