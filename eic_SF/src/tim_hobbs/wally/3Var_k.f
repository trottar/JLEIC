...
C VARIABLE DECLARATIONS
	IMPLICIT NONE
	INTEGER ix,nk,ith,ik,FLAG,typ,dis
	PARAMETER (nk=100)
...
!	L = 0.85                ! for t-exp Lambda value in GeV 
	L = 1.33		! for s-exp Lambda value in GeV
...
!---------------------------------------------------------------------
C***********************************************************************
! WE NOW COMPUTE CONVOLUTIONS AND DISTRIBUTIONS OVER THE RELEVANT |k|
	ik = 1
	kmin = 0.05D0
	kmax = 0.1D0
	kint = (kmax-kmin)/nk
...
! DEFINE THE BOUNDS OF THE theta_h INTEGRAL -- theta_1 TO theta_2
	  ith = 0
	  thmin = 0.D0 /180.D0 * pi          !FULLY INTEGRATE OVER pi
	     thmax = 180.D0 /180.D0 * pi     !BOUNDS IN HAD. PROD. ANGLE
	     thint = (thmax-thmin)/100.D0    !(FROM THIA!) [radians]
	  DO the_h=thmin,thmax,thint


! DEFINE THE BOUNDS OF THE x INTEGRAL -- x1 TO x2
	     ix = 0
	     xmin = 0.02D0
	     xmax = 0.3D0	!SHARP x BOUNDS (FROM THIA!)
	     xint = (xmax-xmin)/100.D0
	  DO x=xmin,xmax,xint
...
!----- FOR THE x INTEGRAL
	    ...
	    ELSE IF (ix/2*2.EQ.ix) THEN
 	    ...
 	    ix = ix + 1
	  ENDDO
	    F2pi_k = (xint/3.D0) * F2pi_k


!----- AND AGAIN FOR THE theta_h INTEGRAL
	     IF (ith.EQ.0) THEN
	      F2piK(ik) = F2piK(ik) + F2pi_k
	    ELSE IF (ith/2*2.NE.ith) THEN
	      F2piK(ik) = F2piK(ik) + 4.D0*F2pi_k
	    ELSE IF (ith/2*2.EQ.ith) THEN
	      F2piK(ik) = F2piK(ik) + 2.D0*F2pi_k
	     ENDIF

 	    ith = ith + 1
	  ENDDO
	    F2piK(ik) = (thint/3.D0) * F2piK(ik)
...
!_____________________________________________________________________________
	  ...
	  IF (ik/2*2.NE.ik) THEN
	     F2pi0 = F2pi0 + 4.D0*F2piK(ik)
	  ELSE IF (ik/2*2.EQ.ik) THEN
	     F2pi0 = F2pi0 + 2.D0*F2piK(ik)
	  ENDIF
	  ik = ik + 1
	ENDDO
	F2pi0 = (kint/3.D0) * F2pi0

	print*, 'First moment of F2piK(k)=',F2pi0
!________________________________________________________________________________________
!________________________________________________________________________________________
C...WRITE DATA TO FILE
!DASSI
          IF (FLAG.EQ.0) THEN
	OPEN (11,FILE='F2pik_INT-x-theta.dat',
     &                           STATUS='UNKNOWN', FORM='FORMATTED')
          ELSE IF (FLAG.EQ.1) THEN
	OPEN (11,FILE='F2rhok_INT-x-theta.dat',
     &                           STATUS='UNKNOWN', FORM='FORMATTED')
          ENDIF
	  DO ik=2,nk
	  WRITE (11,*) kpt(ik), F2piK(ik)
	  print *,'y= ',y, kpt(ik), F2piK(ik)
	ENDDO
	CLOSE (11)
