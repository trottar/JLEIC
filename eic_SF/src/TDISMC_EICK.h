...
double uncerWL(double val,int numData){
	...
	return abs(sqrt(stanDev/numData));
}
...
double uncerW(double val,int numData,double weight[]){
	...
	return abs((numWeight/NEvts)*sqrt(stanDev/numData));
}
...
double GetF2kForCut(int NEvts, Int_t IndexData[], Double_t xBjData[],
 	 ...
 	 for (Int_t i = 0; i < NEvts; i++) {
		    ...
		    xBjData[i] < XCutMax) {
      			...
      			return F2N(xBjData[i], Q2Data[i], NEvts);
    		}
  	}
	//*/
  //CHf2->cd();
