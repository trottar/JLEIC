...
$ir = $srunnum = 10000000+100000*($iqq+1)+1000*$ixx;
 

system("rm -f TDISMC_EICK_cpp.d TDISMC_EICK_cpp.so");
system("rm -f TDIS_lund.txt");

system("rm -f batchk.cc");
# Creat jbatch
open(file_handle,">batchk.cc");
print file_handle <<END_OF_FILE1;
void batchk()
{
    gROOT->ProcessLine(".L TDISMC_EICK.cpp+");
    gROOT->ProcessLine("mainxk($x1,$x2,$q1,$q2,$ir)");
}
END_OF_FILE1
    close(file_handle);

system("root -b -q batchk.cc");
