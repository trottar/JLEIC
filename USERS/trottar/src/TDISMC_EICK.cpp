...
int mainxk(double xMin,double xMax, double Q2Min,double Q2Max, double rnum) {
  ...
  //  Int_t rnumber = gRandom->Uniform(10000); // big number in (<10000000000)
	...
	TGraphErrors *Gf2NQ2[20];
	TMultiGraph *mg = new TMultiGraph();

	for (int i = 0; i < 20; i++) {
	  Gf2NQ2[i] = new TGraphErrors();
	}
	

	for (int i = 0; i < 20; i++) {
	  Gf2NQ2[i]->SetTitle(Form("F2k vs Q^{2} xBj[%4.4f:%4.4f]",xminbin[i],xmaxbin[i]));
	  for (int j = 0; j < 10; j++) {
	    F2kMean = GetF2kForCut(NEvts,IndexData,xBjData,Q2Data,F2NData,q2minbin[j],q2maxbin[j],xminbin[i],xmaxbin[i],F2kMeanError);
	    double delX =  uncerWL((xminbin[j] + xmaxbin[j])/2,i), lum = 100e-15, delQ2 = uncerWL((q2minbin[j] + q2maxbin[j])/2,i);	  
	    double uncerVal[] = {.2};//{lum,delX,delQ2};//uncerVal[] = {lum,delX,delQ2,delt}	    	    
	    delf2k = uncerW(F2kMean,i,uncerVal);	    
	    Gf2NQ2[i]->SetPoint(j, (q2minbin[j] + q2maxbin[j])/2, F2kMean);
	    Gf2NQ2[i]->SetPointError(j,0.,delf2k); //FIXME
	  }
	  //Gf2NQ2[i]->Fit("pol1","FQ");
	  Gf2NQ2[i]->SetMarkerSize(1);
	  Gf2NQ2[i]->SetMarkerStyle(20+i);
	  Gf2NQ2[i]->SetMarkerColor(i+1);
	  Gf2NQ2[i]->SetLineColor(i+1);	
	}
	...
	//CG->BuildLegend(0.1,0.1,1.,1.,"x values");
	CG->Print("MultiGraph.png");
	...
	.0,1000.0
	 ...
	 for (int iEvt=0; iEvt<NEvts; iEvt++){
	   if( XDataQ2[IndOrderQ2[iEvt]] != 0 && YDataQ2[IndOrderQ2[iEvt]] != 0 ){
	     ...
	     cout << XCurveQ2[Aux1] << "\t" << YCurveQ2[Aux1] << endl;
	     Aux1++;
	   }
