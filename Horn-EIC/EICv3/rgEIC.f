...
CPER^^^^^^^^^^^^^^^^^^^^^^
      ...
      INTEGER mxEvent
C      PARAMETER (mxEvent = 5000000)
      PARAMETER (mxEvent = 200000)
...
C**** Begin ****
      ...
      do ipercut = 1, npercut
         lpercut(ipercut) = .false.
      enddo

      call initHbook(nhbmem, ntID)
...
C      QMI = 3.7d0
      QMI = 3.5d0
C TH - Max of Q^2 of electron to be generated
C      QMA = 0.25d0
C      QMA = 3.8d0
      QMA = 4.0d0
C      QMA = 1000.0d0
...
C**********************************************************************
      ...
      write(6,*) 'before ariadne init'
      call flush(6)
C Initialize ARIADNE
      CALL ARINITRAPGAP('RAPGAP')
      write(6,*) 'before rapgap call'
      call flush(6)
